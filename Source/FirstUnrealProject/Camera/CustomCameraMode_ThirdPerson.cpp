...
UCustomCameraMode_ThirdPerson::UCustomCameraMode_ThirdPerson()
{
...
void UCustomCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
	...
	if (!bUseRuntimeFloatCurves)
	{
		if (TargetOffsetCurve)
		{
			...
			if (APlayerCharacter* Player = Cast<APlayerCharacter>(GetTargetActor()))
			{
				...
				View.Location += TargetRotation.GetSafeNormal() *Player->CameraDistance;
			}
		}
	}
	else
...
void UCustomCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UCustomCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	TOptional<AActor*> OptionalPPTarget = TOptional<AActor*>();
...
void UCustomCameraMode_ThirdPerson::PreventCameraPenetration(AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
	...
	for (int32 RayIdx = 0; RayIdx < NumRaysToShoot; ++RayIdx)
	{
		...
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(TEXT("IgnoreCameraCollision")))
				{
					bIgnoreHit = true;
...
void UCustomCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UCustomCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
